...
- [x] **Native item and dependency writes share the reset fence.** Authenticated writes use four allowlisted `SECURITY DEFINER` RPCs that reauthorize the current contributor, lock the project, check workflow generation and item version, and append one private idempotency receipt in the same transaction. Exact success replay precedes generation comparison; stale first attempts consume no key. Authenticated direct DML/write policies are removed, while reads and constrained service-role workflows remain intact.
