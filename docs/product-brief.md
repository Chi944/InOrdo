...
The repository now includes an accessible landing page and responsive product
shell backed by a visibly synthetic local fixture. It presents evidence, direct
and downstream impact paths, recovery proposals, one deliberately unapproved
travel action, and empty history/undo states without claiming that production
workflows are connected.

Authentication, persistence, GPT-5.6 calls, server-side graph traversal,
approval mutations, operation history, undo, and production reset remain to be
